...
// 其实就是把OSScontroller拷贝过来删掉没必要的都删除
...
//        LOG.info(dest.getAbsolutePath());
        ...
        String fileUrl = "";
        try {
            // 初始化VOD客户端并获取上传地址和凭证（主要是VodUtil中的main方法拷贝）
        ...
        fileDto.setVod(vod);// 把上传视频转播的vod填入
        fileService.save(fileDto);

        ResponseDto responseDto = new ResponseDto();
        fileDto.setPath(fileUrl);// 返给前端的地址必须是可播放的地址为了自动获取时长
