<template>
  <div>
    ...
    <!--组件的位置并进行渲染:第一个list组件内部定义的回调方法，第二个是category组件的方法list-->
...
<script>
  ...
  export default {
    ...
    data: function() {
      return {
        category: {},//接收单个category
        categorys: [], //接收category的数组
      }
    },
    mounted: function() {
    ...
    methods: {
      ...
      add() {
        let _this = this;
        _this.category = {}; // 解决新增数据时模态框保留上次数据问题
      ...
      edit(category) {
        let _this = this;
        // vue中将表格行数据显示到表单，反过来也会的问题：$.extend({},category)解决(category,复制给空对象)
        _this.category = $.extend({}, category); //vue中的_tis.category会通过v-modal属性和form表单做数据绑定
        $("#form-modal").modal({backdrop:"static"}, "show"); // 点空白不会关闭
