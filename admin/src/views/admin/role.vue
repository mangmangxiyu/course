<template>
  <div>
    ...
    <!--组件的位置并进行渲染:第一个list组件内部定义的回调方法，第二个是role组件的方法list-->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      edit(role) {
        let _this = this;
        // vue中将表格行数据显示到表单，反过来也会的问题：$.extend({},role)解决(role,复制给空对象)
        _this.role = $.extend({}, role); //vue中的_tis.role会通过v-modal属性和form表单做数据绑定
        $("#form-modal").modal({backdrop:"static"}, "show"); // 点空白不会关闭
